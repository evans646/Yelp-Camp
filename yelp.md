...
There is a  package called ejs-mate that will help us structure our app and add some functionalities as well 
...
and in the views dir we can make a new dir layouts AND MAKE A FILE AND THEN we can pass the body inside the file    <%-body -%>
...
To make the footer stick to the bottom of the page, we give the body a d-flex and flex-column and set a height on the body to be vh-100 \\this give the the body a min height of vh-100
...
CLOUDINARY_SECRET=2SDtDwAypatVhoM5Bm___**LVhq**_220
...
||DEPLOYING||
Now the applications is ready to be released but we connect it to the same database as we are in dev mode, in production we need a database that will run on the server because once the app is deployed it runs on a server somewhere 
...
first we need to go to [https://signup.heroku.com/account] and sign up for  new account.
...
it sets up the app now we are going to deploy it.
...
which makes us a url, 
https://morning-oasis-67101.herokuapp.com/ | https://git.heroku.com/morning-oasis-67101.git

and you can even see it on your heroku account dashboard : https://dashboard.heroku.com/apps 
...
and we can make a new file .gitignore in the top level of the application and put in this code : node_modules 
.env  \\basically telling the app to ignore the .env file and the node_modules we don't want git to care about these files, we just want push the app to heroku 
...
if that does not work right away, you need to add the remote file, you can check here [[https://devcenter.heroku.com/articles/git]

the first command we issued is: git add .\\ to add the files 
and then : git commit -m " deploying"
...
so in the json file, there is a script section, we can add in something else:
{
  ...
  now we can run git commit -m "Aadd start script"
   ...
   which builds the app on heroku but thats not going to work because but it's not going to make the app ready right away, we have another error 

So we can check for the errors: 'Cannot create a client without an access token'

   because the env variables are not configured so to configure it,
   you need to go on to the heroku configuration dashboard => settings and down the settings you will see "reveal config vars" where you can add in more env variables and it will automatically be added on to the app env vars.
 after the env vars are correctly configured, your app will be hosted.

You can also configure your env vars from the command line: heroku config:set and some var name 
